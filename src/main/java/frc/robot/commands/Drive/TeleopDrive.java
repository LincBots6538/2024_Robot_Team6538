...
public class TeleopDrive extends Command {
  ...
  public void execute() {
     ...
     // squaring the inputs 
      double speed_f = Math.signum(speed)*Math.pow(Math.abs(speed), k_DRIVE.SPEED_EXP);
      double turn_f = Math.signum(turn_db)*Math.pow(Math.abs(turn_db), k_DRIVE.TURN_EXP);
